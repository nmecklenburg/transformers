...
class TemperatureLogitsWarper(LogitsProcessor):
    ...
    def __init__(self, temperature: float, local_temperatures: Optional[list[float]] = None, device: str = 'cpu'):
...
class BottomPLogitsWarper(LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        # Remove tokens with cumulative bottom_p above the threshold (token with 0 are kept)
        sorted_indices_to_remove = cumulative_probs >= self.bottom_p
        # Keep at least min_tokens_to_keep
        sorted_indices_to_remove[..., -self.min_tokens_to_keep :] = 0
